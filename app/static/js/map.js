...
import {playerClass, getPlayerX, getPlayerY, setPlayerPosition as setGlobalPlayerPosition, initialiserTalents, utiliserTalent, isCombatActif, getMonstreActuel, getPvMonstre, setCombat, talents, setPlayerMap } from './player.js';
  ...
  fetch(`${API_BASE_URL}/rencontre?x=${getPlayerX()}&y=${getPlayerY()}&carte=${currentMap}`)
    ...
    .then(data => {
      if (data.monstre) {
        ...
        const difficulte = monstre.difficulte || 1;
        
        // Les PV sont maintenant calculés côté backend (pvMax), plus besoin de hardcodage ici.
        // On s'assure juste que pv et pvMax sont bien définis.
        const pvScaled = monstre.pvMax || 30;
        monstre.pv = monstre.pv || pvScaled;
        
        // Stocker le multiplicateur de dégâts pour les talents
        // Diff 1 = x1.0, Diff 5 = x1.4, Diff 10 = x1.9
        monstre.damageMultiplier = 1 + (difficulte - 1) * 0.1;
        
        setCombat(true, monstre, monstre.pv);
        console.log(`⚔️ Combat contre ${monstre.nom} (${monstre.pv} PV, diff ${difficulte}, dmg x${monstre.damageMultiplier.toFixed(1)}) !`);
        
        // Conteneur du monstre avec barre de vie
        const monstreContainer = document.createElement('div');
        monstreContainer.id = "combat-monstre-container";
        monstreContainer.style.position = "absolute";
        monstreContainer.style.left = `${getPlayerX() * tileSize}px`;
        monstreContainer.style.top = `${getPlayerY() * tileSize}px`;
        monstreContainer.style.width = `${tileSize}px`;
        monstreContainer.style.zIndex = "15";
        
        // Barre de vie du monstre
        const healthBarContainer = document.createElement('div');
        healthBarContainer.id = "monstre-health-bar";
        healthBarContainer.style.cssText = `
          width: 100%; height: 8px; background: #333; border-radius: 4px;
          margin-bottom: 2px; overflow: hidden; border: 1px solid #000;
        `;
        const healthFill = document.createElement('div');
        healthFill.id = "monstre-health-fill";
        ...
        // Nom du monstre
        const nomMonstre = document.createElement('div');
        nomMonstre.style.cssText = `
          color: white; font-size: 10px; text-align: center;
          text-shadow: 1px 1px 2px black; margin-bottom: 2px;
        `;
        nomMonstre.textContent = `${monstre.nom} (Niv.${difficulte})`;
        ...
        monstreContainer.appendChild(nomMonstre);
        monstreContainer.appendChild(healthBarContainer);
        monstreContainer.appendChild(monstreDiv);
        document.getElementById("map-inner").appendChild(monstreContainer);
...
export function movePlayer() {
  ...
  if (monsterDiv && !isCombatActif()) {
    ...
    if (playerX === monstreX && playerY === monstreY) {
      ...
      setCombat(true, getMonstreActuel(), getPvMonstre());
...
export function handleKeydown(e) {
  if (isCombatActif() && !["&", "é", '"', "'", "(", "-", "è", "_", "ç", "à"].includes(e.key)) return;
...
export function chargerNouvelleCarte(nomMap, spawnX = null, spawnY = null) {
  ...
  currentMap = nomMap;
  setPlayerMap(nomMap); // Mise à jour de la carte dans le player controller pour la sauvegarde
    ...
    .then(async mapData => {
      ...
      tilesetImage.onload = () => {
        try {
          ...
          // 1. Priorité aux coordonnées fournies (Save ou Transition)
          if (spawnX !== null && spawnX !== undefined && spawnY !== null && spawnY !== undefined && spawnX >= 0 && spawnY >= 0) {
            targetX = Math.max(0, Math.min(spawnX, mapData.width - 1));
            targetY = Math.max(0, Math.min(spawnY, mapData.height - 1));
          } 
          // 2. Sinon, chercher un objet 'player_start' (Nouvelle Partie)
          else {
            const spawn = mapData.layers.find(l => l.name === "player_start")?.objects?.[0];
            if (spawn) {
              targetX = Math.floor(spawn.x / mapData.tilewidth);
              targetY = Math.floor(spawn.y / mapData.tileheight);
            } else {
              // 3. Fallback ultime (0,0) pour éviter le crash
              console.warn("⚠️ Pas de point de départ trouvé, spawn en (0,0) par défaut.");
              targetX = 0;
              targetY = 0;
            }
          }
          ...
          if (!setPlayerPosition(targetX, targetY)) {
            console.error("⚠️ Impossible de placer le joueur sur la case cible (bloquée). Tentative de force...");
            // Si setPlayerPosition échoue (case bloquée), on force l'affichage quand même ou on cherche une case libre voisine (déjà fait dans setPlayerPosition)
            // Si ça échoue vraiment, le joueur risque d'être coincé, mais au moins la carte est chargée.
          }
